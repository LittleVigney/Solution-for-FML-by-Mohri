...
\begin{enumerate}[label=(\alph*), left=0pt] 
    \item 
    $h^{*}$ can give all correct predictions, so all $h^{*}(x_{i})$ is equal to labels that randomly
    filpping has happend. Compare these with labels received by learners, all differences are
    caused by flipping. So the probability that the label of a training point received by 
    the learner disagrees with the one given by $h^{*}$ is equal to the probability that 
    randomly filpping happen. That is $d(h^{*})=\eta$.

    \item 
    $d(h)$ could be contributed by two possible cases, one is filpping and correct prediction, 
    another is no filpping and wrong prediction. The probability of filpping is $\eta$, correct
    prediction is $1 - R(h)$. The probability of no filpping is $1 - \eta$, wrong prediction is 
    $R(h)$. Compute probability of two parts and sum them then get the target.
